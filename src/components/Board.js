import React, {useState} from "react";

import Square from "./Square";


function Board() {
  const [squares, setSquares] = useState(new Array(100).fill(''))

  squares[40] = 'player1'

  return (
    <>
    <div id ="board-container">
      <div id="board">
        {squares.map((_, i) => {
          // You can pass as many props as you like to components!
          return <Square number={i + 1} player={squares[i]}  />
        })}
        {
        // <Square number="99" />
        // <Square number="98" />
        // <Square number="97" />
        // <Square number="96" />
        // <Square number="95" />
        // <Square number="94" />
        // <Square number="93" />
        // <Square number="92" />
        // <Square number="91" />


        // <Square number="81" />
        // <Square number="82" />
        // <Square number="83" />
        // <Square number="84" />
        // <Square number="85" />
        // <Square number="86" />
        // <Square number="87" />
        // <Square number="88" />
        // <Square number="89" />
        // <Square number="90" />
        // <Square number="80" />


        // <Square number="79" />
        // <Square number="78" />
        // <Square number="77" />
        // <Square number="76" />
        // <Square number="75" />
        // <Square number="74" />
        // <Square number="73" />
        // <Square number="72" />
        // <Square number="71" />


        // <Square number="61" />
        // <Square number="62" />
        // <Square number="63" />
        // <Square number="64" />
        // <Square number="65" />
        // <Square number="66" />
        // <Square number="67" />
        // <Square number="68" />
        // <Square number="69" />
        // <Square number="70" />


        // <Square number="60" />
        // <Square number="59" />
        // <Square number="58" />
        // <Square number="57" />
        // <Square number="56" />
        // <Square number="55" />
        // <Square number="54" />
        // <Square number="53" />
        // <Square number="52" />
        // <Square number="51" />

        // <Square number="41" />
        // <Square number="42" />
        // <Square number="43" />
        // <Square number="44" />
        // <Square number="45" />
        // <Square number="46" />
        // <Square number="47" />
        // <Square number="48" /> 
        // <Square number="49" />
        // <Square number="50" />


        // <Square number="40" />
        // <Square number="39" /> 
        // <Square number="38" />
        // <Square number="37" />
        // <Square number="36" />
        // <Square number="35" />
        // <Square number="34" />
        // <Square number="33" /> 
        // <Square number="32" />
        // <Square number="31" />


        // <Square number="21" />
        // <Square number="22" />
        // <Square number="23" />
        // <Square number="24" />
        // <Square number="25" />
        // <Square number="26" />
        // <Square number="27" />
        // <Square number="28" /> 
        // <Square number="29" />
        // <Square number="30" />


        // <Square number="20" />
        // <Square number="19" />
        // <Square number="18" /> 
        // <Square number="17" />
        // <Square number="16" />
        // <Square number="15" />
        // <Square number="14" /> 
        // <Square number="13" /> {" "}
        // <Square number="12" />
        // <Square number="11" />


        // <Square number="1" player={'player1'}/> <Square number="2" /> <Square number="3" />{" "}
        // <Square number="4" /> <Square number="5" />
        // <Square number="6" /> <Square number="7" /> <Square number="8" />{" "}
        // <Square number="9" /> <Square number="10" />
}
      </div>
      </div>
      </>
     
  );
}

export default Board;